...
export async function POST(req: Request, ctx: RouteContext) {
  try {
    ...
    if (user.role !== "NGO") {
      return forbidden("Only NGOs can approve donations");
    }
    ...
    if (donation.status !== "PENDING_APPROVAL") {
      return badRequest("Only pending donations can be approved");
    }
    ...
    console.error("[NGO APPROVE ERROR]", error);
    return serverError("Failed to approve donation");
  }
}
